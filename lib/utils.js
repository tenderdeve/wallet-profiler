...
/**
 * Calculates wallet age in days from first transaction timestamp.
 * @param {string} firstTxDateStr - ISO date string
 * @returns {number} Age in whole days
 */
export function walletAgeInDays(firstTxDateStr) {
  if (!firstTxDateStr) return 0;
  return Math.floor(
    (Date.now() - new Date(firstTxDateStr)) / 86_400_000
  );
}
